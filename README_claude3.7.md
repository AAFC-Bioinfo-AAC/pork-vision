...
- Automated batch processing of multiple images
- Standardized reporting in CSV format
- Visual annotations for review and validation
...
│   ├── orientation.py   # Image orientation correction
│   ├── measurement.py   # Width, depth measurement functions
│   ├── marbling.py      # Marbling detection functionality
│   ├── colouring.py     # Color standard detection
│   └── utils.py         # Utility functions
├── environment.yml      # Conda environment specification
├── porkvision.sh        # Main execution script
...
The automated image analysis pipeline performs the measurements described above through a sequence of modular stages, as outlined below in the flowchart:
...
- Results saved to multiple CSV files by measurement type
- Annotated images showing measurements
- ROI files for manual review or correction of measurements
...
- All images to be processed are in the `data/` directory
- The required models are available in the `src/models/` directory:
  - Segmentation model: `src/models/Yolo_MuscleFatSegment_98epoch.pt`
  - Color detection model: `src/models/color_100_last.pt`

To execute the pipeline on a cluster with SLURM workload manager:
    
```bash
sbatch porkvision.sh
```

For local execution (if supported by your system):
...
| `--debug`             | Output extra information/images                      | `False`                            |
...
| **Variable**          | **Description**                        | **Default Value** |
|-----------------------|----------------------------------------|-------------------|
| `confidence_threshold` | Minimum confidence score for detection | `0.4`             |
...
| `clip_limit`      | CLAHE contrast limit                 | `2.0`                      |
| `tile_grid_size`  | CLAHE tile grid size                 | `(8, 8)`                   |
| `base_filename`   | The image name                       | `None`                     |

#### Coloring Variables
| **Variable**   | **Description**                                       | **Default Value**                  |
|----------------|-------------------------------------------------------|------------------------------------|
| `class_to_std` | YOLO classes and their corresponding color standards  | `Inverted (class 0 = standard 6)`  |
...
- **annotated_images/** – Images with visualized measurements (e.g., `annotated_103_LdLeanColor.JPG`)
...
Each subfolder contains results for each processed image, organized by image name. CSV files provide summary tables for batch analysis.

## Troubleshooting

- Ensure all dependencies are correctly installed via the conda environment
- Check the debug folder for detailed logs if processing fails
- Verify that input images follow the expected format and naming convention
- If model loading fails, check that all model files are present in the src/models directory
